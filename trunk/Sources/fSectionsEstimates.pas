...
type
  TFrameSectionsEstimates = class(TFrame)
...
uses DataModule, DrawingTables, CardSectionsEstimates;

const
  CaptionFrame = 'Фрейм «Разделы смет»';

{$R *.dfm}

  // ---------------------------------------------------------------------------------------------------------------------

constructor TFrameSectionsEstimates.Create(AOwner: TComponent);
begin
...
procedure TFrameSectionsEstimates.TableFilling;
begin
  try
    ...
    begin
      Active := False;
      SQL.Clear;
      SQL.Add('SELECT * FROM sections_estimates ORDER BY code;');
      Active := True;
    end;
...
procedure TFrameSectionsEstimates.VSTAfterCellPaint(Sender: TBaseVirtualTree; TargetCanvas: TCanvas; Node: PVirtualNode;
  Column: TColumnIndex; CellRect: TRect);
var
  CellText: string;
begin
...
procedure TFrameSectionsEstimates.VSTBeforeCellPaint(Sender: TBaseVirtualTree; TargetCanvas: TCanvas;
...
procedure TFrameSectionsEstimates.VSTFocusChanged(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex);
begin
...
procedure TFrameSectionsEstimates.VSTGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex;
...
procedure TFrameSectionsEstimates.VSTResize(Sender: TObject);
begin
...
procedure TFrameSectionsEstimates.PMAddClick(Sender: TObject);
begin
  FormCardSectionsEstimates.ShowForm(-1, ADOQueryTemp);
  TableFilling;
end;

// ---------------------------------------------------------------------------------------------------------------------

procedure TFrameSectionsEstimates.PMEditClick(Sender: TObject);
begin
  FormCardSectionsEstimates.ShowForm(ADOQuery.FieldByName('id').AsInteger, ADOQueryTemp);
  TableFilling;
end;

// ---------------------------------------------------------------------------------------------------------------------

procedure TFrameSectionsEstimates.PMDeleteClick(Sender: TObject);
begin
  if not ADOQuery.Active or (ADOQuery.RecordCount <= 0) then
  begin
  ...
  if MessageBox(0, PChar('Удалить выделенную строку?'), CaptionFrame, MB_ICONINFORMATION + MB_YESNO + mb_TaskModal)
    ...
    try
      ...
      begin
        Active := False;
        SQL.Clear;
        SQL.Add('DELETE FROM sections_estimates WHERE id = :id;');
