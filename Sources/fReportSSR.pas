...
const SSRItems: array[0..14, 0..3] of string =
  (('90.10','9010','СРЕДСТВА НА НЕПРЕДВИДЕННЫЕ РАБОТЫ И ЗАТРАТЫ','0'),
   ...
   ('90.13','9013','ИТОГО НА ДАТУ РАЗРАБОТКИ СМЕТНОЙ ДОКУМЕНТАЦИИ','1'),
   ('90.14','9014','СРЕДСТВА, УЧИТЫВАЮЩИЕ ПРИМЕНЕНИЕ ПРОГНОЗНЫХ ИНДЕКСОВ ЦЕН В СТРОИТЕЛЬСТВЕ НА ДАТУ НАЧАЛА СТРОИТЕЛЬСТВА','0'),
   ('90.15','9015','ИТОГО НА ДАТУ НАЧАЛА СТРОИТЕЛЬСТВА','1'),
   ('90.16','9016','СРЕДСТВА, УЧИТЫВАЮЩИЕ ПРИМЕНЕНИЕ ПРОГНОЗНЫХ ИНДЕКСОВ ЦЕН В СТРОИТЕЛЬСТВЕ В НОРМАТИВНЫЙ СРОК СТРОИТЕЛЬСТВА','0'),
   ('90.17','9017','ИТОГО ПО СВОДНОМУ СМЕТНОМУ РАСЧЕТУ','1'),
   ('90.19','9019','ВОЗВРАТНЫЕ СУММЫ','0'),
   ('90.20','9020','ДОЛЕВОЕ УЧАСТИЕ В СТРОИТЕЛЬСТВЕ','0'),
   ('90.30','9030','ВСЕГО ПО СВОДНОМУ СМЕТНОМУ РАСЧЕТУ','1'));
      VozvratMatStr = '          возвратные материалы';
...
begin
  ...
  //Флаг необходимости расчета упрощенной системы налогооблажения
  FFlagYCN := qrObject.FieldByName('Fl_NAL_USN').AsInteger > 0;
...
begin
  ...
  mtSSRSCID.Value := 0;
  mtSSRItog.Value := 1;   //Итог по главе
  mtSSR.Post;
  ...
  begin
    ...
    mtSSRCID.Value := LastC;
    mtSSRSCID.Value := 0;
    mtSSRItog.Value := 3;  //Итог возврата по строке
    mtSSR.Post;
  end;
  ...
  begin
    ...
    mtSSRCID.Value := LastC;
    mtSSRSCID.Value := 0;
    mtSSRItog.Value := 2; //Итог по главам
    mtSSR.Post;
    ...
    begin
      ...
      mtSSRCID.Value := LastC;
      mtSSRSCID.Value := 0;
      mtSSRItog.Value := 4; // итог по возвратам
      mtSSR.Post;
    end;
  end;
end;
    ...
    begin
      ...
      begin
        ...
        TmpLine.Other :=
          ...
          Line1003.Total - Line1004.Total +
          TmpLine1.Total{V901201+V901202}) * TmpKoef1.Pers);
        TmpLine.CalcTotal;
        AssignLineIfNoNull(TmpLine);
        TmpLine1.Summ(TmpLine);
        ...
        TmpLine.Other :=
          ...
          Line0908.Total) *
          (TmpKoef.Pers * TmpKoef.Koef1 * TmpKoef.Koef2)) *
          TmpKoef1.Pers);
        TmpLine.CalcTotal;
        AssignLineIfNoNull(TmpLine);
        TmpLine1.Summ(TmpLine);
