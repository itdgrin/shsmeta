...
//    FOutline: TTreeView;
  ...
  public
    ...
    procedure AddTab(AReport: TfrxReport; const TabName: String; AFreeObjects: Boolean);
...
{$ENDIF}
  ...
  public
    ...
    procedure AddPreviewTab(AReport: TfrxReport; const TabName: String; const TabCaption: String = ''; FreeObjects: Boolean = True);
    procedure AddPreviewTabOrSwitch(AReport: TfrxReport; const TabName: String; const TabCaption: String = ''; FreeObjects: Boolean = True);
...
begin
  ...
  begin
    ...
    PreviewPages.ObjectOver(PageNo, X, Y, mbLeft, [], Zoom,
      PageBounds.Left - FOffset.X, PageBounds.Top - FOffset.Y, Cur, meMouseMove);
    Cursor := Cur;
  end;
end;
...
begin
  ...
  begin
    PageBounds := FPageList.GetPageBounds(PageNo, ClientWidth, Zoom, FRTLLanguage);
      ...
      PageBounds.Left - FOffset.X, PageBounds.Top - FOffset.Y, Cur, meMouseUp);
    if (GetTickCount - FTimeOffset <= GetDoubleClickTime) then
    begin
      FTimeOffset := 0;
      PreviewPages.ObjectOver(PageNo, X, Y, Button, Shift, Zoom,
      PageBounds.Left - FOffset.X, PageBounds.Top - FOffset.Y, Cur, meDbClick);
    end
    else
    begin
      FTimeOffset := GetTickCount;
      PreviewPages.ObjectOver(PageNo, X, Y, Button, Shift, Zoom,
      PageBounds.Left - FOffset.X, PageBounds.Top - FOffset.Y, Cur, meClick);
    end;
  end;
end;
...
procedure TfrxPreview.AddPreviewTab(AReport: TfrxReport; const TabName: String; const TabCaption: String; FreeObjects: Boolean);
begin
  ...
  FTabItems.AddTab(AReport, TabName, FreeObjects);
  FTabs.Visible := FTabs.Tabs.Count > 1;
  FTabs.TabIndex := FTabs.Tabs.Count - 1;
  OnChangeTab(nil);
end;

procedure TfrxPreview.AddPreviewTabOrSwitch(AReport: TfrxReport; const TabName,
  TabCaption: String; FreeObjects: Boolean);
begin
  ...
  begin
    ...
    begin
      if AReport.FileName <> '' then
        AddPreviewTab(AReport, ExtractFileName(AReport.FileName), TabCaption, FreeObjects)
      else
        AddPreviewTab(AReport, AReport.ReportOptions.Name, TabCaption, FreeObjects);
    end
    else
      AddPreviewTab(AReport, TabName, TabCaption, FreeObjects);
  end;
end;
...
procedure TfrxPreviewTabs.AddTab(AReport: TfrxReport; const TabName: String; AFreeObjects: Boolean);
begin
