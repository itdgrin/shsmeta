...
// text fits, no need to wrap it
  ...
  begin
    ...
    {$IFDEF FPC}
    if (i + 1 <= n) then
      if (Byte(s[i + 1]) > $7F) and AddNext then
      begin
        Inc(addI);
        AddNext := False;
      end
      ...
      {$IFNDEF FPC} and (Win32Platform = VER_PLATFORM_WIN32_NT) {$ENDIF} then
        if FWysiwyg then
          ...
          ExtTextOutExtra(C, X + Round(add1 * SinA), Y + Round(add1 * CosA),
            FOptions, @FScaledRect, PChar(s) + j, i - j, @FTempArray^[j])
          {$ELSE}
          ...
          {$IFDEF FPC}
          ExtTextOutExtra(C, X + Round(add1 * SinA), Y + Round(add1 * CosA),
            FOptions, @FScaledRect, PChar(s) + j , i - j, nil)
          {$ELSE}
          ExtTextOutW(C.Handle, X + Round(add1 * SinA), Y + Round(add1 * CosA),
        ...
        if FWysiwyg then
          {$IFDEF FPC}
          ExtTextOutExtra(C, X + Round(add1 * SinA), Y + Round(add1 * CosA),
            FOptions, @FScaledRect, PChar(AnsiStr) + j, i - j , @FTempArray^[j])
          {$ELSE}
