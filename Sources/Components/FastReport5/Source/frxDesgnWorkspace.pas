...
uses frxRes, frxDMPClass, frxUtils{$IFNDEF FPC}, frxCtrls{$ENDIF};
    ...
    if c is TfrxBand then
      ...
      begin
        ...
        begin
          ...
          Pen.Width := 1;
          Brush.Style := bsClear;
          x := Round((c.Left - FBandHeader) * FScale);
          Rectangle(x, 0, Round((c.Left + c.Width) * FScale) + 1, Round((c.Height) * FScale));
          ...
          begin
            ...
            FillRect(Rect(x + 1, 1, Round(c.Left * FScale), Round(c.Height * FScale)));
          end;
        ...
        begin
          ...
          if (Page is TfrxReportPage) and (TfrxReportPage(Page).Columns > 1) then
            if BandNumber in [4..16] then
              TfrxBand(c).Width := TfrxReportPage(Page).ColumnWidth * fr01cm;
          Pen.Style := psSolid;
          Pen.Color := clGray;
          Pen.Width := 1;
          Brush.Style := bsClear;
          y := Round((Top - FBandHeader) * FScale);
          Rectangle(0, y, Round(c.Width * FScale) + 1, Round((c.Top + c.Height) * FScale) + 1);
          ...
          begin
            ...
            if c is TfrxDataBand then
              ...
              Brush.Color := clBtnFace;
            FillRect(Rect(1, y + 1, Round(c.Width * FScale), Round(c.Top * FScale)));
          end;
            ...
            begin
              ...
              if FScale > 0.7 then
                frxResources.MainButtonImages.Draw(FCanvas,
                  Round(c.Width * FScale - w - 24), Round(y + 2 * FScale), 53);
              if s <> '' then
                TextOut(Round(c.Width * FScale - w - 3), y + 3, s);
            end;
      ...
      with c, FCanvas do
        ...
        begin
          ...
          DrawLine(AbsLeft, AbsTop + c.Height - 3, 0, 3);
          DrawLine(AbsLeft, AbsTop + c.Height, 4, 0);
          DrawLine(AbsLeft + c.Width - 3, AbsTop, 3, 0);
          DrawLine(AbsLeft + c.Width, AbsTop, 0, 4);
          DrawLine(AbsLeft + c.Width - 3, AbsTop + c.Height, 3, 0);
          DrawLine(AbsLeft + c.Width, AbsTop + c.Height, 0, -4);
        end;
...
begin
  ...
  begin
    ...
    if cc is TfrxReportComponent then
...
{  for i := 0 to FObjects.Count - 1 do
  ...
  for i := 0 to SelectedCount - 1 do
    if not FMouseDown then
      DrawSqares(TfrxComponent(FSelectedObjects[i]));
end;
...
begin
  ...
  begin
    cc := TfrxComponent(FObjects[i]);
    if cc is TfrxBand then
      TossObjects(TfrxBand(cc))
    else if cc is TfrxDialogControl then
      AdjustParent(cc, i);
  end;
...
var
  ...
  begin
    ...
    w3 := w2;
    if cc is TfrxBand then
      if TfrxBand(cc).Vertical then
        w0 := FBandHeader
      else
        w2 := FBandHeader;

    Left := cc.AbsLeft;
    Right := cc.AbsLeft + cc.Width;
    Top := cc.AbsTop;
    Bottom := cc.AbsTop + cc.Height;
    mx := X / FScale;
    my := Y / FScale;
    ...
    if (cc is TfrxLineView) and TfrxLineView(cc).Diagonal and
      (cc.Width <> 0) and (cc.Height <> 0) then
    begin
      k := cc.Height / cc.Width;
      if Abs((k * (mx - cc.AbsLeft) - (my - cc.AbsTop)) * cos(arctan(k))) < 5 then
        Result := True;
