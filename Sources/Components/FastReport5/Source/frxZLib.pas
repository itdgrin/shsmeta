...
function inflateInit_(var strm: TZStreamRec; version: PAnsiChar;
  recsize: Integer): Integer; {$IFNDEF FPC}forward;{$ENDIF}
function inflate(var strm: TZStreamRec; flush: Integer): Integer; {$IFNDEF FPC}forward;{$ENDIF}
function inflateEnd(var strm: TZStreamRec): Integer; {$IFNDEF FPC}forward;{$ENDIF}
function deflateInit_(var strm: TZStreamRec; level: Integer; version: PAnsiChar;
  recsize: Integer): Integer; {$IFNDEF FPC}forward;{$ENDIF}
function deflate(var strm: TZStreamRec; flush: Integer): Integer; {$IFNDEF FPC}forward;{$ENDIF}
function deflateEnd(var strm: TZStreamRec): Integer; {$IFNDEF FPC}forward;{$ENDIF}
{****************************************************************************}
...
begin
  Result := zlib.adler32(adler, PByteF(buf), len);
end;
...
function compressBound(sourceLen: LongInt): LongInt;
begin
  Result := zlib.compressBound(sourceLen);
end;

function inflateInit_(var strm: TZStreamRec; version: PChar;
  recsize: Integer): Integer; // forward;
begin
  Result := zlib.inflateInit_(strm, version, recsize);
end;
...
function deflateInit_(var strm: TZStreamRec; level: Integer; version: PChar;
  recsize: Integer): Integer; // forward;
begin
  Result := zlib.deflateInit_(strm, level, version, recsize);
end;
...
begin
  Result := 0;
  {$warning frxZLib, must test inflateReset !}
  ...
  try
    ...
    try
      ...
      begin
        ...
        {$IFDEF FPC}
        zstream.next_out := PByteF(PChar(PtrInt(outBuffer) + zstream.total_out));
        {$ELSE}
