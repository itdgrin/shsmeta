...
begin
  ...
  Result := 0;
  Move(Pointer(frxInteger(@Base) + Offset)^, Result, N);
end;

procedure SetNBytesAt(var Base; Offset, N, Value: Integer);
begin
  Assert(SizeOf(Value) >= N);
  Move(Value, Pointer(frxInteger(@Base) + Offset)^, N);
end;
...
begin
  ...
  begin
    ...
    Move(Pointer(frxInteger(@Data) + Offset)^, FChunk[Used], n);
...
begin
  ...
  for i := 0 to 7 do
    Move(FState[i, 0], Pointer(frxInteger(@Digest) + i*8)^, 8);
end;
