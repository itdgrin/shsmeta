...
function ReplaceControlCodes(var text: string) : string;
begin
    if Pos('&1;', text) = 1 then
    begin
          Delete(text,1,3);
          text := #232 + text;
    end;
    result := StringReplace(text, '&1;', string(#29), [rfReplaceAll]);
end;
...
begin
    ...
    e := GetEncodation(text, textOffset, textSize, data, extCount, last.dataSize, false);  // если кодировка Auto, то вернет самую короткую длину кода
                                                                                           // иначе длину кода для текущей кодировки или -1, если не умещается
