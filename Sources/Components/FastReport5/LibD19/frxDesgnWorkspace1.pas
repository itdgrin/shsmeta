...
var
  ...
  begin
    with FSelectionRect do
      Result := not ((cc.Left > Right / FScale) or
                     (cc.Left + cc.Width < Left / FScale) or
                     (cc.AbsTop > Bottom / FScale) or
                     (cc.AbsTop + cc.Height < Top / FScale));
  end;
    ...
    with FSelectionRect do
      ...
      begin
        ...
        if TObject(FSelectedObjects[0]) is TfrxCustomMemoView then
          FInplaceObject := TfrxCustomMemoView(FSelectedObjects[0]);
      end;
