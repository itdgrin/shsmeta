...
type
  ...
  public
    ...
    function CreateFont(f: TFont): LongInt;
  ...
  private
    ...
    procedure Convert(Sheet: TBiffSheet);
...
function frxConvertMatrixToBiffSheet(po: TfrxBiffPageOptions;
  ShowProgress: Boolean): TBiffSheet;
...
function frxConvertMatrixToBiffSheet(po: TfrxBiffPageOptions; ShowProgress: Boolean): TBiffSheet;
var
...
procedure TfrxBiffConverter.Convert(Sheet: TBiffSheet);
var
  r, c:     Integer;
  obj:      TfrxIEMObject;
  id:       LongInt;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Source.Y + Y > BiffMaxRow + 1 then
        Y := BiffMaxRow + 1 - Source.Y;
  ...
  try
    with po.Matrix do
      ...
      begin
        with po.Source do
          if po.PageBreaks and (GetCellYPos(Y + r) > GetPageBreak(CurPage)) then
          begin
...
function TfrxBiffConverter.GetSheet: TBiffSheet;
begin
...
function TfrxBiffStyles.CreateFont(f: TFont): LongInt;
var
  font: TBiffFont;
