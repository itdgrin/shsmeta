...
type
    ...
    TrueTypeFont = class(TTF_Helpers)
      // Fields
      private beginfile_ptr: Pointer;
      public checksum_action: ChecksumFaultAction;
      private cmap_table: CmapTableClass;
      private dir: TableDirectory;
      private font_header: FontHeaderClass;
      private glyph_table: GlyphTableClass;
      private gsub_table: GlyphSubstitutionClass;
      private horizontal_header: HorizontalHeaderClass;
      private horizontal_metrix_table: HorizontalMetrixClass;
      private index_to_location: IndexToLocationClass;
      private kerning_table: KerningTableClass;
      private name_table: NameTableClass;
      private postscript: PostScriptClass;
      private profile: MaximumProfileClass;
      private selector_ptr: Pointer;
      private windows_metrix: OS2WindowsMetricsClass;
      private preprogram_table: PreProgramClass;
      private ListOfTables: TList;
      private ListOfUsedGlyphs: TList;

      // Methods
      public constructor Create(bgn: Pointer; font: Pointer; action: ChecksumFaultAction);
      destructor Destroy; override;
      public procedure AddCharacterToKeepList(ch: WideChar);
      private procedure BuildGlyphIndexList(used_glyphs: TList; uniscribe: boolean; decompose: boolean; collate: boolean; use_kerning: boolean; var Indices: TIntegerList; var GlyphWidths: TIntegerList);
      private function CalcTableChecksum(font: Pointer; entry: TrueTypeTable; debug: boolean): Cardinal;
      private procedure CalculateFontChecksum(start_offset: Pointer; font_length: Cardinal);
      private procedure ChangeEndian;
      private procedure CheckTablesChecksum;
//      public procedure GetOutlineTextMetrics(var FTextMetric: OutlineTextMetricW);
      private function GetTablesOrder: TTagList;
      private procedure LoadCoreTables;
      private procedure LoadDescriptors(skip_array: TIntegerList);
      public function PackFont(translate_to: FontType; uniscribe: boolean): TByteArray;
      public procedure PrepareFont(skip_list: TIntegerList);
      private procedure ReorderGlyphTable(position: Pointer; uniscribe: boolean);
      private procedure SaveDescriptors(position: Pointer);
      private procedure set_UsedGlyphs(dict: TList);
...
implementation uses SysUtils;
    ...
    procedure TrueTypeFont.BuildGlyphIndexList(used_glyphs: TList; uniscribe: boolean; decompose: boolean; collate: boolean; use_kerning: boolean; var Indices: TIntegerlist; var GlyphWidths: TIntegerList);
    var
        location: Cardinal;
        composed_idx: Word;
...
//              GlyphWidth := 0;
  ...
  {$OVERFLOWCHECKS ON}
    ...
    var
      ...
      i:  Integer;
    begin
      if (self.checksum_action <> ChecksumFaultAction_IgnoreChecksum) then
...
{$IFDEF DRAWING_MANUALLY}
    ...
    begin
        ...
        self.BuildGlyphIndexList(text_as_array, false, false, false, false, @(text_as_array), @(text_widths));
...
{$ENDIF}
    begin
        ...
        self.BuildGlyphIndexList(self.ListOfUsedGlyphs, uniscribe, true, true, false, composite_indexes, glyph_widths);
