...
begin
  ...
  for i := 0 to View.Report.PagesCount - 1 do
  begin
    p := View.Report.Pages[i];
    if (p is TfrxReportPage) and (View.Page <> p) then
...
begin
  FillVariablesList(View.Report, ReportPageParamCB.Items);
  FillPagesList;
...
begin
  ...
  begin
    ...
    begin
      ...
      p := View.Report.FindObject(View.Hyperlink.DetailPage) as TfrxReportPage;
      if p <> nil then
        p.Visible := False;
    end
    else if CustomRB.Checked then
